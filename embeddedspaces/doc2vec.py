...
from embeddedspaces.word2vec import train_sg_pair,train_cbow_pair,queue_to_list,train_prepossess
...
def build_keras_model_dbow(index_size,vector_size,
    ...
    point_model = Sequential()
    point_model.add(Embedding(context_size, vector_size, input_length=sub_batch_size))

    index_model = Sequential()
    index_model.add(Embedding(context_size, vector_size, input_length=sub_batch_size))

    merge_point = Input(name='point', shape=(vector_size,))
    merge_index = Input(name='index', shape=(vector_size,))
    merge_inputs = [index_model(merge_index), point_model(merge_point)]

    ##merged = keras.layers.multiply([merge_point, merge_index])
    ##kerasmodel = Model(inputs=[merge_point, merge_index], outputs=[core])
    ##kerasmodel.compile('rmsprop', {'core':'mse'})
    #
    #merge_model = Sequential()
    #merge_model.add(Merge([index_model, point_model], mode='sum'))
    #merge_model.add(Reshape((vector_size,)))
    #merge_model.add(Activation('sigmoid'))
    ##config = merge_model.to_yaml()
    ##merge_model = model_from_yaml(config)
    #merge_model.compile(loss='mse', optimizer='rmsprop')
    #eturn merge_model

    #index_input = Input(name='index', shape=(sub_batch_size,), dtype='int32')
    #index_embed = Embedding(index_size, vector_size, input_length=sub_batch_size)(index_input)
    #index_model = Model(index_input, index_embed)
    #merge = Lambda(lambda x:x.sum(2)), name='merge',inputs=[embedindex,embedpoint], merge_mode='mul'
    
    #point_input = Input(name='point', shape=(sub_batch_size,), dtype='int32')
    #point_embed = Embedding(context_size, vector_size, input_length=sub_batch_size)(point_input)
    #point_model = Model(point_input, point_embed)

    merged = keras.layers.Multiply()(merge_inputs)#[index_embed, point_embed])
    core = Dense(1, activation='sigmoid', name='core')(merged)
    merged_model = Model([merge_index, merge_point], core)
    merged_model.compile(optimizer='rmsprop', loss='mse')
    return merged_model
...
def build_keras_model_dm(index_size,vector_size,vocab_size,
    ...
    index_input = Input(name='index' , shape=(1,), dtype='int64')
    if doctag_vectors is None :        
        embedindex = Embedding(index_size, vector_size, input_length=1, name='embedindex')(index_input)
    else:
        embedpoint = Embedding(context_size, vector_size, input_length=1, weights=[hidden_vectors], name='embedpoint')(point_input)

    iword_input = Input(name='iword' , shape=(maxwords,), dtype='int64')
    if word_vectors is None :
        embedindex = Embedding(index_size, vector_size, trainable=learn_words, input_length=learn_words, name='embedword')(iword_input)
    else:
        embedindex = Embedding(index_size, vector_size, trainable=learn_words, input_length=learn_words, weights=[word_vectors], name='embedword')(iword_input)

    point_input = Input(name='point' , shape=(sub_batch_size,), dtype='int64')
    if hidden_vectors is None :
        embedpoint = Embedding(context_size, vector_size, trainable=learn_hidden, input_length=sub_batch_size, name='embedpoint')(point_input)
    else:
        embedpoint = Embedding(context_size, vector_size, trainable=learn_hidden, input_length=sub_batch_size, weights=[hidden_vectors], name='embedpoint')(point_input)
        
    if cbow_mean:
        merge = keras.layers.average([embedindex, embedword])
    else:
        merge = keras.layers.add([embedindex, embedword])

    merge2 = keras.layers.dot([merge, embedpoint])
    sig = Activation('sigmoid')(merge2)

    kerasmodel = Model(inputs=[index_input, iword_input, point_input], outputs=sig)
    kerasmodel.compile('rmsprop', {'activation_1':'mse'})
...
class Doc2Vec(gensim.models.doc2vec.Doc2Vec):
    def train(self, docs=None,
        ...
        if self.sg:
            ...
            self.kerasmodel.fit_generator(gen,steps_per_epoch=samples_per_epoch, epochs=self.iter,verbose=0)
        else:
            if self.dm_concat:
...
class SentenceClassifier(BaseEstimator, ClassifierMixin):
    ...
    def predict(self,X):
        ...
        if self.sents_shuffle :
            s_indexs=list(range(len(self.sents_all)))
            random.shuffle(s_indexs)
            s_invers_indexs=list(range(len(s_indexs)))
...
class Doc2VecClassifier(SentenceClassifier):
    ...
    def doc2vec_set(self,all_docs):
        ...
        doc2vec_init_dict={k:vars(self)[k] for k in list(vars(self).keys())  if k in doc2vec_init_param_dict}
...
if __name__ == "__main__":
    ...
    for d in doc1:
        ...
        break;
    d_size=5
    dv1=Doc2Vec(doc1,size=d_size,dm=0,dm_concat=1,hs=0,negative=5,iter=1)
